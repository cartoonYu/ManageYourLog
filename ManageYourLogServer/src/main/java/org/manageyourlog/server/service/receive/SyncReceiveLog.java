...
public class SyncReceiveLog extends AbstractReceiveLog {
    ...
    protected boolean judgeParamIllegal(UploadLogRecordReq uploadLogRecordReq) {
        ...
        if(Objects.isNull(uploadLogRecordReq.getContent())){
            log.error("sync receive log, upload req content is null, data: {}", GsonUtil.getInstance().writeJson(uploadLogRecordReq));
            return false;
        }
        if(Objects.isNull(uploadLogRecordReq.getOperator())){
            log.error("sync receive log, upload req operator is null, data: {}", GsonUtil.getInstance().writeJson(uploadLogRecordReq));
            return false;
        }
        if(Objects.isNull(uploadLogRecordReq.getLogRecordSort())){
            log.error("sync receive log, upload req log record sort is null, data: {}", GsonUtil.getInstance().writeJson(uploadLogRecordReq));
            return false;
        }
        return true;
    }
