...
import org.manage.log.test.base.util.DefineHttpResponseUtil;
...
import org.manage.log.upload.service.http.UploadLogInterface;
...
public class UploadLogByHttpTest extends BaseTest{
    ...
    public void testUploadSingleLog(){
        ...
        Mockito.when(uploadLogInterface.uploadSingleLog(ArgumentMatchers.eq(uploadLogRecordReq))).thenReturn(DefineHttpResponseUtil.defineHttpResponse(new OperateLogResp<>(true)));
    ...
    public void testUploadLogList(){
        UploadLogRecordReq uploadLogRecordReq = DefineModelUtil.defineLogRecordReq();
        Mockito.when(uploadLogInterface.uploadLogList(ArgumentMatchers.eq(ImmutableList.of(uploadLogRecordReq)))).thenReturn(DefineHttpResponseUtil.defineHttpResponse(new OperateLogResp<>(true)));
